...
Vector* vector_init(unsigned int initial_capacity, size_t type_size)
{
  ...
  result->array = calloc(result->capacity, type_size);
  result->size = 0;
  result->type_size = type_size;
...
Vector* vector_copy(Vector* vector, Copy copy_value)
{
  ...
  result->type_size = vector->type_size;

  for(int i = 0; i < result->size && copy_value; i++)
    result->array[i] = copy_value(get(vector, i));
...
void vector_print(Vector* vector, Print print_value)
{
  ...
  else 
  {
    ...
    if(get(vector, vector->size-1) == NULL)
      printf("NULL");
    else
      print_value(get(vector, vector->size-1));
    printf("]\n");
  }
